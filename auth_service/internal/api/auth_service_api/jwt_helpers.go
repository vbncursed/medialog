...
// getUserRoleFromContext извлекает роль пользователя из JWT токена
func (a *AuthServiceAPI) getUserRoleFromContext(ctx context.Context, jwtSecret string) (string, error) {
	tokenString, err := extractTokenFromContext(ctx)
	if err != nil {
		return "", err
	}
	...
	role, ok := claims["role"]
	if !ok {
		return "user", nil // По умолчанию "user"
	}
