...
func (l *LibraryServiceAPI) GetEntry(ctx context.Context, req *pb_models.GetEntryRequest) (*pb_models.EntryResponse, error) {
	...
	res, err := l.libraryService.GetEntry(ctx, req.GetEntryId(), userID)
	if err != nil {
		switch {
		...
		case errors.Is(err, library_service.ErrUnauthorized):
			return nil, newError(codes.PermissionDenied, ErrCodeUnauthorized, "You don't have permission to access this entry.")
		default:
